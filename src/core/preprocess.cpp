...
PointCloudXYZITPtr Preprocess::lidarConvert(const livox_ros_driver2::msg::CustomMsg::SharedPtr& msg)
{
    /*
    순서
        1. 점 유효성 검사
        ...
        6. 중복점 제거
    */
   auto cloud = std::make_shared<PointCloudXYZIT>();
...
PointType Preprocess::makePoint(const livox_ros_driver2::msg::CustomPoint& src) const
{
    PointType pt;
    pt.x = src.x;
    pt.y = src.y;
...
bool Preprocess::isTooClose(const PointType& pt) const
{
...
bool Preprocess::isDuplicatePoint(const PointType& pt, const PointType& prev_pt, bool has_prev) const
{
    if(!has_prev)
        return false;
...
void Preprocess::finalizeCloud(const PointCloudXYZITPtr& cloud) const
{
