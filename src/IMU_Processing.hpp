...
/* 
LiDAR 1프레임 + imu 묶음인 measures 받아서
1. 초기 IMU 정렬/초기화 하고
2. 이후 프레임부터는 IMU로 상태 예측
3. LiDAR 포인트를 undistortion하기
4. 그 결과를 feats_undistort로 넘기기
 */
class ImuProcess
{
 public:
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    //EKF로 IMU기반 상태 예측 + 공분산 propagation
    // 직전까지 데이터인 IMU의 각속도/가속도를 써서, 현재는 이런 자세.위치.속도 일것이라고 예측해봄.
    //공분산 : 내 상태 추정이 얼마나 불확실한지 수치로 가진 행렬
    // propagation: 현재 불확실성을 다음 시점으로 넘겨 계산함. 
