...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  /*fast-lio2는 feature_enabled가 false임 */
  if (feature_enabled)
  {
  ...
  // line 범위 체크
  // tag로 유효한 점인지 확인
  // point_filter_num으로 샘플링 : 점 너무 많으면 리소스 커지니, 전체 점 중에서 3의 배수 번째 point만 받음
  // 좌표/반사도/시간 복사 
  // 이전 점과 완전 같은 점은 제거
  // blind 거리 이내 점 제거
  // 통과한 점을 pl_surf에 넣음
  else
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      {
        valid_num++;
        /*리소스 줄이기 위해서 3의 배수 번째만 받음*/
        if (valid_num % point_filter_num == 0)
        {
          pl_full[i].x = msg->points[i].x;
          pl_full[i].y = msg->points[i].y;
          pl_full[i].z = msg->points[i].z;
          pl_full[i].intensity = msg->points[i].reflectivity; //반사도
          pl_full[i].curvature = msg->points[i].offset_time / //시간
                                 float(1000000);  // use curvature as time of each laser points, curvature unit: ms
          //이름만 곡률(curv~)이지, 실제는 상대 시간을 넣음. 
          //찰나의 순간의 이동으로 점 위치가 달라지니,  그 점이 찍힌 시간, 그 순간의 IMU기반 자세/위치를 이용해서,
          //  나중에 모든 점을 같은 기준 시각으로 되돌려서 정렬함.(undistortion)
          // 예를 들어서 스캔 시작 timestamp가 10.000초이고, 어떤 점은 offset_time이 0.045초이면, 10.045초쯤 찍힌거다. 
          // 상대적으로 나중에 찍힌 점을 IMU값과 함께 motion undistortion한다.

          /*이전 점과 같은 점이면 버리자. +  LiDAR 원점에서 너무 가까운 점은 무시. */
          //blind는 .yaml에서 0.5로 정의됨.
          // 즉 0.5보다 먼 점 일때만, 쌓임. 
